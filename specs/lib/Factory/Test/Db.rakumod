...
sub parse-database-url(Str:D $url --> Hash) is export {
  ...
  if %out<adapter> eq 'sqlite' {
    my $path = $rest;
    $path = $path.substr(2) if $path.starts-with('//');
...
sub connect-database(Str:D $url) is export {
  ...
  given %c<adapter> {
    ...
    when 'sqlite' {
      DBIish.connect: 'SQLite', database => ':memory:';
    }
    default { die "unsupported adapter: %c<adapter>" }
  }
}
